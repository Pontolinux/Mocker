...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision :shell, privileged: false, inline: <<-EOS
    curl -O https://raw.githubusercontent.com/wckr/mocker-bashrc/master/bashrc && mv -f bashrc ~/.bashrc && source ~/.bashrc
    docker pull mocker/mocker:latest
    mocker run --name mocker
  EOS
